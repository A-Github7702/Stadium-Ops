...
This application is a premium, interactive 3D Command-Center Dashboard built for FIFA World Cup stadium operations. It integrates real-time WebGL geometries, voice-controlled camera routing, AI-powered congestion analysis via Gemini, and a global emergency SOS evacuation grid.
...
* **Alignment Checklist**:
  ...
  * **AI Route Waypoint Analysis**: When a congested gate is selected, a Vercel serverless function calls Gemini 3.5 Flash server-side with live occupancy data to generate a reasoned congestion warning and reroute recommendation — dynamically, not from a template.
  * **Multimodal Voice Assistant**: Uses the **Web Speech API** to parse keywords and trigger camera sweeps to gates and hubs.
  * **Gemini Vision Prototype**: Processes dropped image metadata (signages, corridors, sector hazards) as a prototype interaction demonstrating a vision-based workflow (keyword-based simulation, not live image analysis).
...
* **Structure & Readability**: The project features a modular React component architecture:
  * [App.jsx](src/App.jsx): Manages global telemetry JSON states and binds system overrides.
  * [StadiumCanvas.jsx](src/components/StadiumCanvas.jsx): Encapsulates 3D rendering, mesh definitions, paths, and camera animations.
  * [AIInputHub.jsx](src/components/AIInputHub.jsx): Isolates speech recognition and vision simulation algorithms.
  * [TelemetryControl.jsx](src/components/TelemetryControl.jsx): Encapsulates the UI sliders and raw JSON viewer.
  * [EmergencyControl.jsx](src/components/EmergencyControl.jsx): Renders override controls.
  * [api/analyze.js](api/analyze.js): Vercel serverless function that securely calls Gemini server-side, keeping the API key out of client code.
...
* **Server-Side API Key Isolation**: The Gemini API key was moved out of client-side code entirely and into a Vercel serverless function (`api/analyze.js`). The key is never bundled into the browser, never sent in client requests, and never visible in dev tools — verified via network inspection.
...
* **Automated Test Suite**: 4 unit/integration tests written with Vitest and React Testing Library, covering:
  * Gate occupancy slider correctly updates state on change
  * A gate above 80% occupancy is correctly flagged as "heavy" status
  * A gate at 0% occupancy is correctly flagged as "clear" status
  * Graceful fallback UI when the Gemini API call fails or returns no data
  * Run via `npm run test` — all 4 tests passing.
...
```bash
    npm run build
    # Compiles with zero errors into dist/
```

---

## 6. Accessibility (Low Impact)
* **ARIA Compliance**: Integrated semantic landmarks (`<header>`, `<main>`, `<aside>`, `<button>`) to help screen readers.
